# Save=Ctrl+S
# Run=Alt+N you can change ir=t under preferences->keyboard shortcuts
...
# F stands for formating and within paranthesis any valid expression is accepted

n1_full = F"{first} {last}"
n2_full = f"{len(first)} {2+2}"
print(n1_full)
print(n2_full)

# Using . to open methods(predefined functions) for the variables
course = "   Python Programming"
# Here course is an object and upper is a method to convert course into upper string followed by brackets
print(course.upper())
...
# .title() to convert every first alphabet of a word in a string into capital
new_world = "welcome to new world order"
print(new_world)
print(new_world.title())

# .strip() to remove blank spaces from beginnig or from the end of the string
# .lstrip()  from end and .rstrip() from beginning
print(course.strip())

# .find() to find the index of a string or alphabet(starting) within the string and index no starts with 0
print(new_world.find("o"))

# .replace("to be replace character", "new character") within the string
print(full.replace("Singh", "Kaiser"))

# REMEMBER . OPERATOR DOES NOT CHANGES THE ORIGINAL STRING


# in operator to check whether the given sting or character exist within the string
# not in operator is opposite of in operator
# it is a boolean operation i.e it checks whether the character or string exist or not
print("Sin" in full)
print("Vin" not in full)


# NUMBERS

# Complex numbers, x=1+2j where j is iota
# Division a/b gives floating point no and a//b gives integer
# a%b gives remainder
# a**b is a^b
# a=a+b can also be written as a+=b, += is called augmenta assignment operator

# round() turns the float no into nearest no
# abs() turns the negative no into positive no

# Importing math module to perform complex mathematical operations because python has limited inbuild mathematicalfunctions
# Using "import math" command

print(math.ceil(4.2))

# BELOW IS THE MODULE WHICH CAN BE SEARCH ONLINE
# USE '''...........''' to add multi line commnet
...
# Using input fxn to get an input only in the string format from the user

x = input("x:")

# HERE ENTERING 1 I.E NUMERIC VALUE IS ACCEPTED AS "1" , A STRING INSTEAD OF NUMERIC VALUE AND HENCE NO  MATHEMATICAL OPERATIONS  CANNOT BE PERFORMED ON X

# TO GET INPUT FROM THE CUSTOMER open TERMINAL , TYPE python filename.py , in our case it is python app.py

# TYPE CONVERSION IS USED TO CHANGE THE DATA TYPE OF X
# int(x) , converts x into a integer type
# Similarly float(x), bool(x) and str(x)
...
# print(f"x: {value of x} , y: {}) where {} will show the value of the variable in the output

print(f"x: {x}, y: {y}")

# bool() is used to interpret falsy and truly values
# In boolean context ""(empty string),0 and None is considered as falsy values anything else will be true
# I.E bool(values other than "",0 and None) will convert into TRUE

# FRUIT="APPLE" , PRINT(FRUIT[1:-1]) , -1 IS THE CHARACTER AT THE END OF STRING AND ALWAYS THE CHARACTER AT END INDEX NO WILL NOT BE DISPLAYED IN CASEE OF STRING
